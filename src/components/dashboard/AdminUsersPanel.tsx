...
const AdminUsersPanel = ({ adminId, isSessionActive }: { adminId: string; isSessionActive: () => boolean }) => {
  ...
  return (
    <Card>
      <CardHeader><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="flex items-center gap-2"><UserCog className="h-5 w-5 text-primary" />Personas y permisos</CardTitle><CardDescription>Cambia roles o revoca accesos. Las cuentas no se eliminan.</CardDescription></div><Badge variant="secondary">{users.length} cuentas</Badge></div></CardHeader>
