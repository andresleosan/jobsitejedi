...
const AdminUsersPanel = ({ adminId, isSessionActive }: { adminId: string; isSessionActive: () => boolean }) => {
  ...
  return (
    <Card className="admin-panel-card admin-users-card">
      <CardHeader className="admin-panel-header"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="admin-panel-title flex items-center gap-2"><UserCog className="h-5 w-5 text-primary" />Personas y permisos</CardTitle><CardDescription>Cambia roles o revoca accesos. Las cuentas no se eliminan.</CardDescription></div><Badge variant="secondary">{users.length} cuentas</Badge></div></CardHeader>
