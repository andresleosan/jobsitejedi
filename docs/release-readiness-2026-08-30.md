...
Estado: gate local aprobado; producción sin modificar; despliegue aún no autorizado.

## Alcance preparado

- Jerarquía única `admin > manager > builder`, con invitaciones privilegiadas solo para admin.
...
4. Repetir inventario productivo de solo lectura y confirmar si existen documentos que requieran
   los backfills de asignación/jornada. Si aplica migración, verificar backup y rollback antes.
...
3. Ejecutar preflight productivo de solo lectura, backup aplicable y checklist de rollback.
4. Solicitar autorización explícita para el despliegue de ese SHA y enumerar exactamente Rules,
   índices, Functions y frontend incluidos. App Check continúa en observación.
5. Desplegar por componentes, ejecutar smokes negativos/positivos y monitorear errores/costo.
