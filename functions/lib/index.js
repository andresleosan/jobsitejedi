...
const normalizeAccessRequestHistoryRecordIds = (value) => {
    if (value == null)
        return null;
    if (!isRecord(value)) {
        throw new https_1.HttpsError("invalid-argument", "History cleanup details are invalid");
    }
    if (value.recordIds === undefined)
        return null;
    if (!Array.isArray(value.recordIds)) {
        throw new https_1.HttpsError("invalid-argument", "History record ids must be an array");
    }
    if (value.recordIds.length > 200) {
        throw new https_1.HttpsError("invalid-argument", "Too many history records selected");
    }
    return [...new Set(value.recordIds.map((recordId) => {
            if (typeof recordId !== "string" || !/^[A-Za-z0-9_-]{1,200}$/.test(recordId.trim())) {
                throw new https_1.HttpsError("invalid-argument", "History record id is invalid");
            }
            return recordId.trim();
        }))];
};
const isTerminalAccessRequest = (data, requestId) => accessRequestIsCurrent(data, requestId)
    && (data.status === "approved" || data.status === "rejected");
const isSameAccessRequestVersion = (historyData, currentData) => historyData.requestedAt instanceof firestore_1.Timestamp
    && currentData.requestedAt instanceof firestore_1.Timestamp
